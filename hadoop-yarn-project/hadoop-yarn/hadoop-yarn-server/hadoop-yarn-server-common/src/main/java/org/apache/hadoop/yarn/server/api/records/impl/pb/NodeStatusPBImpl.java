...
public class NodeStatusPBImpl extends NodeStatus {
    ...
    private synchronized void addSqueezedContainersToProto() {
        maybeInitBuilder();
        builder.clearSqueezedContainers();
        if (squeezedContainers == null)
            return;
        Iterable<ContainerSqueezeUnitProto> iterable = new Iterable<ContainerSqueezeUnitProto>() {
            @Override
            public Iterator<ContainerSqueezeUnitProto> iterator() {
                return new Iterator<ContainerSqueezeUnitProto>() {
                    ...
                    Iterator<ContainerSqueezeUnit> iter = squeezedContainers.iterator();
    ...
    private synchronized void initSqueezedContainers() {
        if (this.squeezedContainers != null) {
            return;
        }

        NodeStatusProtoOrBuilder p = viaProto ? proto : builder;
        List<ContainerSqueezeUnitProto> list = p.getSqueezedContainersList();
        this.squeezedContainers = new ArrayList<ContainerSqueezeUnit>();

        for (ContainerSqueezeUnitProto c : list) {
            this.squeezedContainers.add(convertFromProtoFormat(c));
        }
    }
    ...
    @Override
    public synchronized void setSqueezedContainers(List<ContainerSqueezeUnit> squeezedContainers) {
        maybeInitBuilder();
        if ( squeezedContainers == null) {
            builder.clearSqueezedContainers();
        }
    ...
    @Override
    public synchronized List<ContainerSqueezeUnit> getSqueezedContainers() {
    ...
    private ContainerSqueezeUnitPBImpl convertFromProtoFormat(ContainerSqueezeUnitProto c) {
        return new ContainerSqueezeUnitPBImpl(c);
    }
