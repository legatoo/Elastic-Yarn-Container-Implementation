...
public class QueueMetrics implements MetricsSource {
    ...
    //TODO: add variable to present squeezedMB
    private Map<NodeId, Resource> squeezedMBInNodes =
            new HashMap<NodeId, Resource>();

    private Map<NodeId, Resource> usedSqueezedMBInNodes =
            new HashMap<NodeId, Resource>();
    ...
    /**
     * Set squeezed resource to queue. To be called by scheulder periodically as
     * receive squeezed containers from heart beat
     */
    ...
    public synchronized void setUsedSqueezedResourcesToQueue(NodeId nodeId, Resource resource){
        usedSqueezedMBInNodes.put(nodeId, resource);
    }

    public synchronized Resource getSqueezedResourceOnNode(NodeId nodeId){
        if( squeezedMBInNodes.containsKey(nodeId))
            return squeezedMBInNodes.get(nodeId);
        else
            return Resources.none();
    }

    public synchronized Resource getUsedSqueezedResourceOnNode(NodeId nodeId){
        if (usedSqueezedMBInNodes.containsKey(nodeId))
            return usedSqueezedMBInNodes.get(nodeId);
        else
            return Resources.none();
    }
    ...
    public void releaseSqueezedResource(String user, int containers, Resource res){
