...
public class ApplicationMasterService extends AbstractService implements
    ...
    // this is the AM-RM heart beat
    @Override
    public AllocateResponse allocate(AllocateRequest request)
            throws YarnException, IOException {

        LOG.debug(" POWER in AM Service, request is " + request);
    ...
    /* check if its in cache */
        ...
        /* old heartbeat */
            ...
            for (ResourceRequest req : ask) {
                LOG.debug(" yifan. in am service, ask is  " + ask);
            ...
            for (ResourceRequest qq : ask) {
                LOG.debug("FUCK: " + qq);
            }
            // Send new requests to appAttempt.
            // call scheuler's allocate
            Allocation allocation =
                    this.rScheduler.allocate(appAttemptId, ask, release,
                            blacklistAdditions, blacklistRemovals);

            LOG.debug("POWER in AM service, allocation is " + allocation.getContainers());
