...
class CSQueueUtils {
    ...
    @Lock(CSQueue.class)
    public static void updateQueueStatisticsAfterSqueeze(
            final ResourceCalculator calculator,
            final CSQueue childQueue, final Resource diff,
            final Resource clusterResource, final NodeId nodeId) {

        Resource currentSqueezedResource = childQueue.getAvailableSqueezedResource();
        Resource availableSqueeze = Resources.add(currentSqueezedResource, diff);

        childQueue.getMetrics().setAvailableSqueezedResourcesToQueue(nodeId,
                Resources.max(
                        calculator,
                        clusterResource,
                        availableSqueeze,
